...
def create_venue_submission():
  ...
  try:
    ...
    if number == 0:
      ...
      flash(f'This venue is already listed {venue_form.name.data}, please try again')
  except:  
    db.session.rollback()
    print('DB rollback')
    print(sys.exc_info())
    flash(f'There was a problem recording the venue {venue_form.name.data}, please try again')
  finally:  
    print('db closed')
    db.session.close()
...
def search_artists():
  ...
  search_by = request.form.get('search_term')
  search_result = Artist.query.filter(Artist.name.ilike(f'%{search_by}%')).all()
  response=[]
